...
import com.te.ecommerce.service.FlipcartServiceImpl;
...
public class FlipcartController {
	...
	public String getHomePage(Integer id, String password, HttpServletRequest request, ModelMap map) {
		...
		if(admin != null) {
			...
			map.addAttribute("message", "invalid id or password");
			return "login";
		}
	}
	...
	public String addItem(ModelMap map, Item item) {
		...
		if(service.addItem(item)) {
			...
			map.addAttribute("message", "Item  exists");
		}
		return "addItem";
	...
	public String getAllItems(ModelMap map) {
		...
		if(list != null) {
			...
			map.addAttribute("message", "there is no items...");
			return "searchAll";
		}
	}
